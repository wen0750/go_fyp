...
// define the JSON representation of the response
...
// define the JSON representation of a request object
type request struct {
    Raw               []string           `json:"raw,omitempty"`
    CookieReuse       bool               `json:"cookie-reuse,omitempty"`
    Method            string             `json:"method,omitempty"`
...
// define the JSON representation of a matcher object
type matcher struct {
    Type      string   `json:"type,omitempty"`
    Part      string   `json:"part,omitempty"`
    Words     []string `json:"words,omitempty"`
...
// define the JSON representation of an extractor object
...
// define the JSON representation of a workflow object
type workflow struct {
    Template      string         `json:"template,omitempty"`
    Subtemplates  []subtemplate `json:"subtemplates,omitempty"`
}

// define the JSON representation of a subtemplate object
type subtemplate struct {
    Tags string `json:"tags,omitempty"`
}

// receive YAML data and convert it into file download response
func serveYAML(c *gin.Context) {
    ...
    // send the received data as JSON for debugging
    response := responseData{
        ID:        yamlData.ID,
        Info:      yamlData.Info,
        Requests:  yamlData.Requests,
        Workflows: yamlData.Workflows,
    }
    ...
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Unable to marshal YAML"})
        return
    }

    // set the appropriate headers to trigger a download in the browser
    c.Writer.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=%s.yaml", yamlData.ID))
    c.Writer.Header().Set("Content-Type", "application/x-yaml")
    c.Writer.Write(yamlBytes)

    // save the data to MongoDB
    mongodb.InsertData(yamlBytes)
}
...
func main() {
    ...
    // use POST method to receive YAML data from Website
    router.POST("/editor", serveYAML)
