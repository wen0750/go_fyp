...
import (
	"go_fyp/core/backend/services/database/cve"
	"net/http"
	"strings"
...
func Action_ListAll(request *gin.Context) {
	...
	result, err := cve.ListAll()
	if err != nil {
		request.JSON(http.StatusInternalServerError, gin.H{"error": "Error listing cve", "errors": err})
		return
	}
...
func Action_ListOne(request *gin.Context) {
	...
	regexp, _ := regexp.Compile("CVE-[0-9]{4}-[0-9]{4,5}")
	...
	// get data from database
	result, err := cve.ListOne(cveid)
	if err != nil {
		request.JSON(http.StatusInternalServerError, gin.H{"error": "Error listing cve", "code": http.StatusInternalServerError, "errorCode": "1004"})
		return
	}
